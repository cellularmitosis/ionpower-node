...
curl http://leopard.sh/misc/beta/ionpower-node-0.92-g3-ppc.tar.gz | gunzip | tar x
...
│   └── share/cacert.pem
└── ionpower-node-0.92/          <- Node-compat runtime (statically links OpenSSL 1.1.1t)
    └── bin/node                 <- expects sibling mozjs
```

The fastest path: grab prebuilt tarballs. The runtime is attached to
the [v0.92 release](https://github.com/cellularmitosis/ionpower-node/releases/tag/v0.92);
...
curl -L -O https://github.com/cellularmitosis/ionpower-node/releases/latest/download/ionpower-node-0.92-g3-ppc.tar.gz
