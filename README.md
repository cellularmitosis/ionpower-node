...
│   └── share/cacert.pem
└── ionpower-node-0.86/          <- Node-compat runtime (statically links OpenSSL 1.1.1t)
    └── bin/node                 <- expects sibling mozjs
```

The fastest path: grab prebuilt tarballs. Both the runtime and the
matching SpiderMonkey are attached to the [v0.86 release](https://github.com/cellularmitosis/ionpower-node/releases/tag/v0.86).

```bash
# One-time: SpiderMonkey + IonPower JIT (G3 example)
curl -L -O https://github.com/cellularmitosis/ionpower-node/releases/download/v0.86/mozjs-45-ionpower-g3.tar.gz
...
curl -L -O https://github.com/cellularmitosis/ionpower-node/releases/latest/download/ionpower-node-0.86-g3-ppc.tar.gz
