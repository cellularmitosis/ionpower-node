...
curl http://leopard.sh/misc/beta/ionpower-node-0.91-g3-ppc.tar.gz | gunzip | tar x
cd /opt/ionpower-node-0.91/bin
...
│   └── share/cacert.pem
└── ionpower-node-0.91/          <- Node-compat runtime (statically links OpenSSL 1.1.1t)
    └── bin/node                 <- expects sibling mozjs
```

The fastest path: grab prebuilt tarballs. The runtime is attached to
the [v0.91 release](https://github.com/cellularmitosis/ionpower-node/releases/tag/v0.91);
...
curl -L -O https://github.com/cellularmitosis/ionpower-node/releases/latest/download/ionpower-node-0.91-g3-ppc.tar.gz
sudo tar xzpf ionpower-node-0.91-g3-ppc.tar.gz -C /opt/

/opt/ionpower-node-0.91/bin/node test/hello.js
```
