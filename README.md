...
curl http://leopard.sh/misc/beta/ionpower-node-1.0-g3-ppc.tar.gz | gunzip | tar x
...
│   └── share/cacert.pem
└── ionpower-node-1.0/          <- Node-compat runtime (statically links OpenSSL 1.1.1t)
    └── bin/node                 <- expects sibling mozjs
```

The fastest path: grab prebuilt tarballs. The runtime is attached to
the [v1.0 release](https://github.com/cellularmitosis/ionpower-node/releases/tag/v1.0);
...
curl -L -O https://github.com/cellularmitosis/ionpower-node/releases/latest/download/ionpower-node-1.0-g3-ppc.tar.gz
