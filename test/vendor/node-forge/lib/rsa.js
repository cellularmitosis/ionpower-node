...
pki.rsa.generateKeyPair = function(bits, e, options, callback) {
  ...
  if(!forge.options.usePureJavaScript && !options.prng &&
    ...
    if(callback) {
      ...
      // try native sync — wrapped in try/catch by ionpower-node so we
      // fall through to the pure-JS path when the host's crypto.* is
      // a delegating shim that calls back into us (causing recursion).
      if(_detectNodeCrypto('generateKeyPairSync') && !_crypto.__ionpower_inhibit_rsa) {
        try {
        ...
        } catch (_e) { /* fall through to pure-JS impl */ }
      }
    }
  }
